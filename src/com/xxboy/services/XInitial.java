...
public class XInitial extends AsyncTask<Void, Void, Integer> {
	...
	protected Integer doInBackground(Void... params) {
		...
		Logger.log("checking xcamera cache folder");
		if (!cacheFolder.exists()) {
			Logger.log("creating xcamera cache folder");
			cacheFolder.mkdirs();
		}

		Logger.log("checking xcamera folder");
		if (!xFolder.exists()) {
			Logger.log("creating xcamera folder");
			xFolder.mkdirs();
		}

		File noMediaFlagFile = new File(cacheFolder.getAbsolutePath() + File.separator + ".nomedia");
		Logger.log("checking xcamera cache nomedia flag file");
		if (!noMediaFlagFile.exists()) {
			try {
				Logger.log("creating xcamera cache nomedia flag file");
